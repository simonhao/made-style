...
function Compiler(ast, options){
  ...
  if(Array.isArray(options.external) && options.external.length){
    options.external.forEach(function(external_id){
      var module_path = mid.path(external_id, self.options);
      if(module_path){
        self.external[module_path] = true;
      }
    });
  }
